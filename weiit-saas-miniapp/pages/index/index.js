...
Page({
				...
				requ.request({
					...
					success: function(res) {
						if (res.data.isBargain == true) {
							wx.navigateTo({
								url: '../chopPage/chopPage?bargain_id=' + id +
									'&b_order_id=' + res.data.b_order_id,
							})
						} else {
							wx.navigateTo({
								url: '../address/address?value=' + value +
									'&bargain_id=' + id,
							})
						}
					}
			...
			if (code == 0) {
				if (app.conFig.switchTabUrl != undefined) {
					if (app.conFig.switchTabUrl.indexOf('detail') != -1) {
						...
						wx.navigateTo({
							url: '../detail/detail?shop_id=' + e.detail.target.dataset
								.product_id,
						})
					}
				} else {
	...
	groupBtn: function(e) {
		...
		var scrollTop;
		if (index > 0) {
			scrollTop = ((this.data.group_con[index - 1].productInfo.length - 1) * 35) + (this.data
				.group_con[index - 1].productInfo.length * 75) + ((this.data.group_con[index - 1]
				.productInfo.length - 1) * 1)
		} else {
			scrollTop = 0
		}
		this.setData({
				...
				if (res.code == '0') {
					...
					for (let i = 0; i < layout.length; i++) {
						...
						if (layout[i].type == "advertisement") {
							layout[i].height = 98 / parseFloat((layout[i].pageParam.imgWidth /
								layout[i].pageParam.imgHeight).toFixed(2))
						};
						// 商品分组
	...
	onShareAppMessage: function() {
		...
		return {
			...
			path: 'pages/index/index?page=' + getApp().page + '&share_user_id=' + wx.getStorageSync(
				'user_id')
		}
	},
	// 分享成功
